...
def debug_prediction(samples, m):
    from TripleES import TripleES
    if not isinstance(m, TripleES): # For debug only
        import sklearn as skl
        if isinstance(m['esti'], skl.tree._classes.DecisionTreeClassifier): 
            tree_trace(samples, m)

def tree_trace(X_test, m):
